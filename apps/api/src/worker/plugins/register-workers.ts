...
import { evalFixProcessor } from "../workers/eval-fix";
...
const registerWorkers: FastifyPluginCallback = (fastify, _, done) => {
  ...
  // Pipeline worker remains registered for legacy /sites/scrape onboarding.
  ...
  fastify.queues.evalFix.worker.run(evalFixProcessor(fastify));
