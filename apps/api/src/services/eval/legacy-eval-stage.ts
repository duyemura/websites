// apps/api/src/services/eval/legacy-eval-stage.ts
// Legacy pixel-diff QA stage: compares the original source site to the hosted Milo site.
// Kept for the `milo eval` CLI / join pipeline.
 ...
 * matching the proven capture pattern used during site onboarding.
 *
 ...
 * receives equal-sized buffers — matching the proven approach in the onboarding capture flow.
...
function computeSimilarity(a: Buffer, b: Buffer): { score: number; heightDeltaPx: number } {
  ...
  const diffCount = pixelmatch(cropA.data, cropB.data, undefined, width, height, { threshold: 0.1 });
  const score = Math.round((1 - diffCount / (width * height)) * 100);
  const heightDeltaPx = imgA.height - imgB.height; // positive = origin taller than hosted Milo site
...
/**
 * Build the hosted Milo URL for a page. Prefers the subdomain (e.g. ab867633.mygymseo.com)
...
export const legacyEvalStage: StageRunner = {
  label: "eval",
  ...
  async run(ctx: StageContext): Promise<StageResult> {
    ...
    try {
      for (const page of pages) {
        const hostedMiloPageUrl = hostedMiloUrl(cdnBase, ctx.siteUuid, page.path, cacheBust, ctx.config.MILO_PREVIEW_DOMAIN);
        ctx.log(`  Scoring ${page.path} → ${hostedMiloPageUrl.split("?")[0]} …`);
        try {
          const originPageUrl = `${sourceOrigin}${page.path}`;

          // Screenshot origin and hosted Milo site in parallel (shared browser, separate pages)
          const [miloCapture, originCapture] = await Promise.all([
            screenshotPage(browser, hostedMiloPageUrl),
            screenshotPage(browser, originPageUrl),
          ]);

          const { score, heightDeltaPx } = computeSimilarity(miloCapture.png, originCapture.png);
